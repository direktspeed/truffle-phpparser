...
from bench_export_csv import bintree_ids_php_by_val, bintree_ids_php8_by_val, bintree_ids_jphp_by_val, bintree_ids_hhvm, \
    bintree_ids_trufflephp_native_by_val, bintree_ids_trufflephp_by_val, bintree_ids_php_by_ref, bintree_ids_php8_by_ref, \
    bintree_ids_jphp_by_ref, bintree_ids_trufflephp_native_by_ref, bintree_ids_trufflephp_by_ref, fannkuch_ids_php_by_val, \
    fannkuch_ids_php8_by_val, fannkuch_ids_jphp_by_val, fannkuch_ids_hhvm, fannkuch_ids_trufflephp_native_by_val, \
    fannkuch_ids_trufflephp_by_val, spectral_ids_php_by_ref, spectral_ids_php8_by_ref, spectral_ids_jphp_by_ref, \
    spectral_ids_trufflephp_native_by_ref, spectral_ids_trufflephp_by_ref, spectral_ids_trufflephp_by_val, \
    spectral_ids_trufflephp_native_by_val, spectral_ids_hhvm, spectral_ids_jphp_by_val, spectral_ids_php8_by_val, \
...
def plot_speedup_box(title, save_name, php_val, gphp_val, gphp_native_val, warmup_thres=0):
    ...
    trufflephp_val = gphp_val / php_mean
    trufflephp_native_val = gphp_native_val / php_mean

    php_val = php_val / php_mean

    labels = ['php', 'trufflephp', 'trufflephp-native']
    times = [1 / php_val, 1 / trufflephp_val, 1 / trufflephp_native_val]
...
def plot_speedup(title, save_name, php_val, gphp_val, gphp_native_val, warmup_thres=0):
    ...
    php_mean = statistics.mean(php_val)
    trufflephp_mean = statistics.mean(trufflephp_val)
    trufflephp_native_mean = statistics.mean(trufflephp_native_val)

    vals = [php_val, trufflephp_val, trufflephp_native_val]
    speedup = [php_mean, php_mean / trufflephp_mean, php_mean / trufflephp_native_mean]

    # Calculate the standard deviation
    php_std = statistics.stdev(php_val)
    trufflephp_std = statistics.stdev(trufflephp_val)
    trufflephp_nat_std = statistics.stdev(trufflephp_native_val)

    # Define labels, positions, bar heights and error bar heights
    labels = ['php', 'trufflephp', 'trufflephp-native']
    x_pos = np.arange(len(labels))
    CTEs = speedup
    # CTEs = [fk_php_mean, fk_trufflephp_mean, fk_trufflephp_nat_mean]
    error = [php_std, trufflephp_std, trufflephp_nat_std]
...
def spectralnorm_report_plot():
    ...
    ids = [
        ...
        , spectral_ids_trufflephp_native_by_val  # gphp native
        , spectral_ids_trufflephp_by_val  # gphp
        # by ref
...
def fannkuchredux():
    ids = [
        ...
        , fannkuch_ids_trufflephp_native_by_val  # gphp native
        , fannkuch_ids_trufflephp_by_val  # gphp
    ]
    vals = []
    for ids_bench in ids:
        val_ids = []
