...
def run_trufflephp(bm_dir):
    TRUFFLEPHP_HOME = os.environ.get('TRUFFLEPHP_HOME')
    if not TRUFFLEPHP_HOME:
        out("[!] TRUFFLEPHP_HOME not set to run trufflephp")
        return
    binary = os.path.join(TRUFFLEPHP_HOME, "trufflephp")
    for f in get_files(bm_dir, ".trufflephp"):
        run(binary, f)
    pass
